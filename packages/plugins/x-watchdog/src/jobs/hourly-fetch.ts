...
import { detectSignals, type ProjectConfig } from "../lib/signal-detector.js";
import {
  parseChannelsFromBio,
  toChannelsAvailable,
} from "../lib/lead-enrichment.js";
import {
...
type XTweet = { id: string; text: string; author_id: string; created_at: string };

function toXTweet(t: { id: string; text: string; authorId: string; createdAt: string }): XTweet {
...
export async function handleHourlyFetch(ctx: PluginContext, companyId = ""): Promise<void> {
  ...
  if (batchHandles.length) {
    const lastSeenId = await getDbConfig(db as any, `lastSeen_batch_${batchGroup}`);
    ...
    if (twitterApiIo) {
      ...
      const backfilledJson = await getDbConfig(db as any, backfillKey);
      const backfilledSet = new Set<string>(
        backfilledJson ? JSON.parse(backfilledJson) : []
      );
      ...
      for (const handle of toBackfill) {
        try {
          ...
          ctx.logger.info(
            `[hourly-fetch] backfilled @${handle.username}: ${result.tweets.length} tweets`
          );
        } catch (err) {
      ...
      if (toBackfill.length) {
        await setDbConfig(db as any, backfillKey, JSON.stringify([...backfilledSet]));
      }
    }
  }

  // Keyword-based search (catch mentions from any account)
  const keywordsJson = await getDbConfig(db as any, "search_keywords");
  if (keywordsJson) {
    try {
      const keywords: string[] = JSON.parse(keywordsJson);
      const kwLastSeen = await getDbConfig(db as any, "lastSeen_keywords");
  ...
  if (batchHandles.length) {
    await setDbConfig(db as any, `lastSeen_batch_${batchGroup}`, maxId);
  }
  if (keywordsJson) {
    await setDbConfig(db as any, "lastSeen_keywords", maxId);
  }
  ...
  if (cfg.openaiApiKey) {
    try {
      ...
      const globalPrompt = await getDbConfig(db as any, "scoring_prompt");
      const handleIds = [...new Set(unscoredTweets.map((t) => t.handleId))];
      const projectPrompts: { prompt: string | null }[] = [];
      ...
      const combinedPrompt =
        [globalPrompt, ...projectPrompts.map((r) => r.prompt).filter((p): p is string => !!p)]
          .filter(Boolean)
          .join("\n\n") || undefined;
...
async function detectLeads(ctx: PluginContext, cfg: Awaited<ReturnType<typeof resolveConfig>>, companyId: string): Promise<void> {
  const db = getDb();
  ...
  const projectConfigs: ProjectConfig[] = allProjects
    .filter((p) => p.triggerKeywords)
    .map((p) => ({
      id: p.id,
      name: p.name,
      triggerKeywords: safeParseJson<string[]>(p.triggerKeywords),
      scoringPrompt: p.scoringPrompt,
      outreachChannels: safeParseJson<string[]>(p.outreachChannels),
      outreachTemplates: safeParseJson<Record<string, string>>(p.outreachTemplates),
  ...
  const lastCheckedId = await getDbConfig(db as any, "leads_last_checked_tweet_id");
  ...
  const recentlyScored = db
    ...
    .where(
      and(
        sql`${tweets.aiScore} IS NOT NULL`,
        gte(tweets.aiScore, 0.6),
        lastCheckedId ? sql`${tweets.id} > ${lastCheckedId}` : sql`1=1`
      )
    )
    .orderBy(tweets.id)
    .limit(50)
  ...
  if (!recentlyScored.length) return;

  // Build handle map
  const handleIds = [...new Set(recentlyScored.map((t) => t.handleId))];
  const handleRows: { id: string; username: string }[] = [];
  ...
  if (!signals.length) {
    ...
    await setDbConfig(db as any, "leads_last_checked_tweet_id", maxId);
    return;
  }
  ...
  const newSignals = signals.filter(
    (s) => !existingLeads.has(`${s.tweet.username}:${s.project.id}:${s.tweet.id}`)
  );
  ...
  if (cfg.openaiApiKey) apiKeys["OPENAI_API_KEY"] = cfg.openaiApiKey;
  ...
  for (const signal of toProcess) {
    try {
      ...
      ctx.logger.info(
        `[detect-leads] Created lead: @${signal.tweet.username} → ${signal.project.name} (${contextPack.urgency}, ${contextPack.signal_type})`
      );
      ...
      try {
        await ctx.events.emit(EVENT_NAMES.newLead, companyId, {
          ...
          draftMessage:
            contextPack.drafted_messages?.x_dm ?? contextPack.suggested_action,
        });
      } catch (emitErr) {
      ...
      ctx.logger.error(
        `[detect-leads] Error processing signal for @${signal.tweet.username}: ${err}`
      );
    }
  }
  ...
  if (toProcess.length >= newSignals.length) {
    ...
    ctx.logger.info(
      `[detect-leads] ${newSignals.length - toProcess.length} signals remaining, checkpoint NOT advanced`
    );
  }
}
